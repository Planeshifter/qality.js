...
Examination.Session = function(obj, callback) {
	...
	this.input = function(node) {
        ...
        $(ipdiv).change(function() {
            ...
            if (node.duration > 0) self.deactivate_timer(node);
		...
		for (var i = 0; i < self.sequence.nodes.length; i++) {
		  ...
		  switch(actual.type) {
			  	...
			  	case "input":
			  	  ...
			  	  if (actual.given_answer == actual.right_answer) res.points += actual.points;
			  	break;
		  	}
		  }
