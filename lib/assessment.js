...
Assessment.Input = function(no) {
	...
	this.init = function() {
		...
		s += '<div class = "question">' + self.question + '</div>';
	...
	this.show_setting = function() {
		if ( !self.setting ) {
			...
			s += '<div id = "entries">';
			    ...
			    s += '<select name = selector id = "Transition_In" onchange="window.Input.set_option(this, 0);">';
			    ...
			    s += '<select name = selector id = "Transition_Out" onchange="window.Input.set_option(this, 1);">';
				s += '<option selected = "selected">static</option>';
				s += '<option>dynamic</option>';
				s += '<option>top_down</option>';
				s += '<option>bottom_up</option>';
                ...
                s += '<input id = "node_points" name="vorname" type="text" value="' + self.points + '">';
				...
				s += '<input id = "node_duration" name="vorname" type="text" value="' + self.duration + '">';
...
Assessment.Editor = function() {
	...
	this.keydown = function(event){
		...
		switch(event.keyCode) {
			...
			case 37:
			 if (self.active_sequence) self.previous_node();
			break;	 
				 
				 
			case 39:
			 if (self.active_sequence) self.next_node();
			break;	 
    ...
    var s = '<div id = "load_file">';
    
    s += '<div id = "exit"><img src = "img/exit.svg"></div>';
    
    s += '<input id= "fileInput" type="file" class="fileInput">';
    
    s += '<img id = "Backing" src = "img/box.svg"/>';
    
    
    s += '<hr/>';
    
    s += '<div class = "label">COPY PASTE JSON</div>';
    
    s += '<textarea id = "json_paste" rows="4" cols="50">';
    s += '</textarea>';
    ...
    $("#load_file #exit").click(function(){
       $("#load_file").fadeOut(200, function() { $("#load_file").remove()  }); 
        });
    ...
    this.create_sequence = function(temp) {
    
        self.sequence.evaluation     = temp.evaluation;
