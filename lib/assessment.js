	...
	this.show_setting = function() {
		if (!self.setting) {
			...
			$(".setting #submit").click(function(){
				...
				self.question = $("#node_question").val();
				$(".question").html(marked(self.question) );
				$(".setting").fadeOut();
...
Assessment.Evaluation = function() {
	...
	self.seperator.push( { start: 33.333, id: 0});
	self.seperator.push({ start: 66.666, id: 1 });	
...
Assessment.Editor = function() {
	...
	$('.seperator').draggable({
			...
			if (pos < 0) 		$(this).css("left", "1%");  // if it goes into the negative range	
			if (pos > width )   $(this).css("left", "99%");
			var pos = $(this).position().left;
	...
	this.init = function() {
		...
		s += '<div id = "evaluation_form" class = "evaluation_form">';
			...
			s += '<div class = "tools">';
			  s += '<div id = "plus" class = "tool"><img src = "img/plus.svg"/></div>';
			s += '</div>';			
			...
			for (var n = 1; n < 3; n++) {
				...
				s += '<div no = "' + nr + '" class = "seperator" style="left:' + off + '%" ></div>';
			}
		...
		$(".evaluation_form #plus").click(function(){
		
			var nr = self.evaluation.seperator.length;
			
			
			var s = '<div no = "' + nr + '" class = "seperator" style="left:50%" ></div>';
			$(".evaluation_form .range").append(s);
			
			self.evaluation.seperator.push( { id: nr, start: 0.5 } );
