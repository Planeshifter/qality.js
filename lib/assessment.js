	...
	this.show_setting = function() {
		...
		if ( !self.setting ) {
			...
			$(".setting #submit").click(function(){
				...
				self.points     = parseInt ( $("#node_points").val(), 10 );
...
Assessment.MultipleChoice = function(no) {
	...
	this.interaction = function() {
		...
		$(".mpanswer").click(function(){
			...
			var old = self.answers[self.right_value].points;
	...
	this.init = function() {
		...
		var x = {  text: "A_0", points: pt1, assessment: "ASSESSMENT" };
		self.answers.push(x);

		var y = {  text: "A_1", points: pt2, assessment: "ASSESSMENT" };
		self.answers.push(y);
